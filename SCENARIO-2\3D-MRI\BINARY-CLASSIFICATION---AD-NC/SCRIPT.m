% DISHARMONY BETWEEN BATCH NORMALIZATION AND DROPOUT CASE-2
% ADD ONLY ONE DROPOUT LAYER RIGHT BEFORE THE SOFTMAX LAYER
% WITH RANDOM ZOOM IN + OUT AUGMENTATION
clc
clear all
close all
filters = 12;
layers = [...
    image3dInputLayer([121 145 41],"Name","image3dinput",'Normalization','zerocenter')
    ...
    fullyConnectedLayer(500,"Name","fc_1",'WeightL2Factor', 0.00005, 'BiasL2Factor', 0.00005)
    fullyConnectedLayer(100,"Name","fc_2",'WeightL2Factor', 0.00005, 'BiasL2Factor', 0.00005)
    fullyConnectedLayer(2,"Name","fc_3",'WeightL2Factor', 0.00005, 'BiasL2Factor', 0.00005)
    dropoutLayer(0.1,"Name","dropout")
    softmaxLayer("Name","softmax")
    classificationLayer("Name","classoutput")];

TRAINING_SET = imageDatastore('D:\ahsan\TOBECOPIED\temp\FOLD-6\TRAIN', 'IncludeSubfolders',true, 'LabelSource', 'foldernames', 'FileExtensions','.mat','ReadFcn',@(x) matRead(x));
VALIDATION_SET = imageDatastore('D:\ahsan\TOBECOPIED\temp\FOLD-6\VAL', 'IncludeSubfolders',true, 'LabelSource', 'foldernames', 'FileExtensions','.mat','ReadFcn',@(x) matRead(x));
